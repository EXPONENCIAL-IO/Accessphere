...
class PathInterpolatorDonut implements Interpolator {
    ...
    public PathInterpolatorDonut(Path path) {
        ...
        this.f13mX = new float[numPoints];
        this.f14mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, null);
            this.f13mX[i] = position[0];
            this.f14mY[i] = position[1];
        }
    }
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f13mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f13mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f13mX[endIndex] - this.f13mX[startIndex];
        if (xRange == 0.0f) {
            return this.f14mY[startIndex];
        }
        float fraction = (t - this.f13mX[startIndex]) / xRange;
        float startY = this.f14mY[startIndex];
        return ((this.f14mY[endIndex] - startY) * fraction) + startY;
    }
