...
import android.support.p001v4.animation.AnimatorCompatHelper;
...
public class ItemTouchHelper extends ItemDecoration implements OnChildAttachStateChangeListener {
    ...
    /* renamed from: UP */
    public static final int f23UP = 1;
    int mActionState = 0;
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            if (action == 0) {
                ...
                if (ItemTouchHelper.this.mSelected == null) {
                    ...
                    if (animation != null) {
                        ItemTouchHelper.this.mInitialTouchX -= animation.f24mX;
                        ItemTouchHelper.this.mInitialTouchY -= animation.f25mY;
    ...
    public static abstract class Callback {
        ...
        public void onDraw(Canvas c, RecyclerView parent, ViewHolder selected, List<RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            ...
            for (int i = 0; i < recoverAnimSize; i++) {
                ...
                anim.update();
                int count = c.save();
                onChildDraw(c, parent, anim.mViewHolder, anim.f24mX, anim.f25mY, anim.mActionState, false);
        ...
        public void onDrawOver(Canvas c, RecyclerView parent, ViewHolder selected, List<RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            ...
            for (int i = 0; i < recoverAnimSize; i++) {
                RecoverAnimation anim = (RecoverAnimation) recoverAnimationList.get(i);
                int count = c.save();
                onChildDrawOver(c, parent, anim.mViewHolder, anim.f24mX, anim.f25mY, anim.mActionState, false);
                c.restoreToCount(count);
            }
            if (selected != null) {
                int count2 = c.save();
    ...
    private class RecoverAnimation implements AnimatorListenerCompat {
        ...
        public void update() {
            if (this.mStartDx == this.mTargetX) {
                this.f24mX = ViewCompat.getTranslationX(this.mViewHolder.itemView);
            } else {
                this.f24mX = this.mStartDx + (this.mFraction * (this.mTargetX - this.mStartDx));
            }
            if (this.mStartDy == this.mTargetY) {
                this.f25mY = ViewCompat.getTranslationY(this.mViewHolder.itemView);
            } else {
                this.f25mY = this.mStartDy + (this.mFraction * (this.mTargetY - this.mStartDy));
            }
        }
    ...
    public View findChildView(MotionEvent event) {
        ...
        for (int i = this.mRecoverAnimations.size() - 1; i >= 0; i--) {
            ...
            if (hitTest(view, x, y, anim.f24mX, anim.f25mY)) {
                return view;
            }
        }
