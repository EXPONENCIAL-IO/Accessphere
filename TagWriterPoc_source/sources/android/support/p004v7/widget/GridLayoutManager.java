...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public View findReferenceChild(Recycler recycler, State state, int start, int end, int itemCount) {
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount && getSpanIndex(recycler, state, position) == 0) {
                if (((android.support.p004v7.widget.RecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
    ...
    private void measureChildWithDecorationsAndMargin(View child, int widthSpec, int heightSpec, boolean capBothSpecs) {
        ...
        android.support.p004v7.widget.RecyclerView.LayoutParams lp = (android.support.p004v7.widget.RecyclerView.LayoutParams) child.getLayoutParams();
