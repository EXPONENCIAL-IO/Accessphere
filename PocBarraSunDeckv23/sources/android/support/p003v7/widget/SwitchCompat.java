...
public class SwitchCompat extends CompoundButton {
    ...
    public void setSwitchTextAppearance(Context context, int resid) {
        TypedArray appearance = context.obtainStyledAttributes(resid, C0140R.styleable.SwitchCompatTextAppearance);
        ColorStateList colors = appearance.getColorStateList(C0140R.styleable.SwitchCompatTextAppearance_android_textColor);
        if (colors != null) {
            this.mTextColors = colors;
        } else {
            this.mTextColors = getTextColors();
        }
        int ts = appearance.getDimensionPixelSize(C0140R.styleable.SwitchCompatTextAppearance_android_textSize, 0);
        if (!(ts == 0 || ((float) ts) == this.mTextPaint.getTextSize())) {
            this.mTextPaint.setTextSize((float) ts);
            requestLayout();
        }
        if (appearance.getBoolean(C0140R.styleable.SwitchCompatTextAppearance_textAllCaps, false)) {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int switchWidth = Math.max(this.mSwitchMinWidth, (this.mThumbWidth * 2) + paddingLeft + padding.right);
