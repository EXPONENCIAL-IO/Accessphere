    ...
    public int decode(int[] received, int numECCodewords, int[] erasures) throws ChecksumException {
        ...
        if (erasures != null) {
            ...
            for (int i2 = 0; i2 < length; i2++) {
                int b = this.field.exp((received.length - 1) - erasures[i2]);
                ModulusPoly modulusPoly = new ModulusPoly(this.field, new int[]{this.field.subtract(0, b), 1});
                knownErrors = knownErrors.multiply(modulusPoly);
            }
        }
        ModulusPoly modulusPoly2 = new ModulusPoly(this.field, S);
        ModulusPoly[] sigmaOmega = runEuclideanAlgorithm(this.field.buildMonomial(numECCodewords, 1), modulusPoly2, numECCodewords);
