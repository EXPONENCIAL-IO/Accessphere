...
public final class Code128Reader extends OneDReader {
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints) throws NotFoundException, FormatException, ChecksumException {
        ...
        if (!row.isRange(nextStart2, Math.min(row.getSize(), ((nextStart2 - lastStart) / 2) + nextStart2), false)) {
            ...
            ResultPoint resultPoint = new ResultPoint(left, (float) rowNumber);
            ResultPoint resultPoint2 = new ResultPoint(right, (float) rowNumber);
            Result result = new Result(sb2, rawBytes, new ResultPoint[]{resultPoint, resultPoint2}, BarcodeFormat.CODE_128);
            return result;
        }
    }
}
